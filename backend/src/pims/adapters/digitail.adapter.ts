...
/**
 * Digitail PIMS Adapter
 * Modern cloud-based veterinary practice management with open API.
 * Docs: https://developer.digitail.io/
 */
@Injectable()
export class DigitailAdapter implements PimsAdapter {
  readonly providerName = 'digitail';
  private readonly logger = new Logger(DigitailAdapter.name);
  private readonly baseUrl = 'https://api.digitail.io/v1';

  async syncAvailability(clinicId: string, apiKey: string) {
    this.logger.log(`Syncing availability for Digitail clinic: ${clinicId}`);
    const response = await axios.get(`${this.baseUrl}/practices/${clinicId}/capacity`, {
  ...
  async sendPatientFile(clinicId: string, apiKey: string, patientFile: PatientFile) {
    this.logger.log(`Sending patient file to Digitail clinic: ${clinicId}`, patientFile.petName);
    const response = await axios.post(
      `${this.baseUrl}/practices/${clinicId}/patients`,
      {
  ...
  async getAppointmentSlots(clinicId: string, apiKey: string): Promise<AppointmentSlot[]> {
    this.logger.log(`Getting appointment slots from Digitail clinic: ${clinicId}`);
    const response = await axios.get(`${this.baseUrl}/practices/${clinicId}/slots`, {
      headers: { Authorization: `Bearer ${apiKey}` },
      timeout: 7000,
    });
