...
/**
 * Priza PIMS Adapter
 * Cloud-based veterinary practice management for the Israeli market.
 * Docs: https://priza.co.il/api-docs
 */
@Injectable()
export class PrizaAdapter implements PimsAdapter {
  readonly providerName = 'priza';
  private readonly logger = new Logger(PrizaAdapter.name);
  private readonly baseUrl = 'https://api.priza.co.il/v1';

  async syncAvailability(clinicId: string, apiKey: string) {
    this.logger.log(`Syncing availability for Priza clinic: ${clinicId}`);
  ...
  async sendPatientFile(clinicId: string, apiKey: string, patientFile: PatientFile) {
    this.logger.log(`Sending patient file to Priza clinic: ${clinicId}`, patientFile.petName);
  ...
  async getAppointmentSlots(clinicId: string, apiKey: string): Promise<AppointmentSlot[]> {
    this.logger.log(`Getting appointment slots from Priza clinic: ${clinicId}`);
