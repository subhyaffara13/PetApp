 ...
 */
export const verifyDaaSWebhookSignature = (
  provider: 'wolt_drive' | 'uber_direct',
) => {
  return (
    ...
    const signatureHeader = (req.headers['x-daas-signature'] ||
      req.headers['x-wolt-signature'] ||
      req.headers['x-uber-signature']) as string | undefined;
    const timestampHeader = req.headers['x-daas-timestamp'] as
      | string
      | undefined;
    ...
    if (
      ...
      (!signatureHeader || signatureHeader === 'mock_test_signature')
    ) {
      return next();
    }
    ...
    if (!signatureHeader || Array.isArray(signatureHeader)) {
      return res
        .status(401)
        .json({ error: 'Missing or invalid webhook signature header' });
    }
    ...
    if (timestampHeader && typeof timestampHeader === 'string') {
      ...
      if (Math.abs(currentTime - pingTime) > 300) {
        return res
          .status(403)
          .json({ error: 'Webhook timestamp expired (replay detected)' });
      }
    }
    ...
    const dataToSign = timestampHeader
      ? `${timestampHeader}.${rawPayload.toString('utf8')}`
      : rawPayload;
