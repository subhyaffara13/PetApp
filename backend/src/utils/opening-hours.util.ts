export interface PlaceOpenStatusInput {
  openingHours?: string;
  isOpenNow?: boolean;
  isDeclaredOpen?: boolean;
  portalStatusOverride?: 'open' | 'closed' | 'schedule';
  capacityStatus?: string;
  isClaimed?: boolean;
}

const DAY_MAP: Record<string, number> = {
  su: 0,
  sun: 0,
  sunday: 0,
  mo: 1,
  mon: 1,
  monday: 1,
  tu: 2,
  tue: 2,
  tuesday: 2,
  we: 3,
  wed: 3,
  wednesday: 3,
  th: 4,
  thu: 4,
  thursday: 4,
  fr: 5,
  fri: 5,
  friday: 5,
  sa: 6,
  sat: 6,
  saturday: 6,
};

/**
 * Extracts the current day (0=Sun..6=Sat) and time in minutes from midnight for a given time zone.
 */
export function getLocalTimeDetails(
  date = new Date(),
  timeZone = 'Asia/Jerusalem',
): { day: number; minutes: number } {
  try {
    const formatter = new Intl.DateTimeFormat('en-US', {
      timeZone,
      weekday: 'short',
      hour: 'numeric',
      minute: 'numeric',
      hour12: false,
    });
    const parts = formatter.formatToParts(date);
    const weekdayStr =
      parts.find((p) => p.type === 'weekday')?.value.toLowerCase().slice(0, 3) ||
      'sun';
    const hourVal = parts.find((p) => p.type === 'hour')?.value || '0';
    const minuteVal = parts.find((p) => p.type === 'minute')?.value || '0';

    const hour = parseInt(hourVal, 10);
    const minute = parseInt(minuteVal, 10);

    return {
      day: DAY_MAP[weekdayStr] ?? date.getDay(),
      minutes: (hour % 24) * 60 + minute,
    };
  } catch {
    return {
      day: date.getDay(),
      minutes: date.getHours() * 60 + date.getMinutes(),
    };
  }
}

/**
 * Parses day range tokens like "Sun–Thu", "Mon-Fri", "Fri", "Daily" into an array of day indices (0..6).
 */
function parseDaysFromSegment(segmentText: string): number[] | null {
  const lower = segmentText.toLowerCase();

  if (lower.includes('daily') || lower.includes('everyday') || lower.includes('all days')) {
    return [0, 1, 2, 3, 4, 5, 6];
  }

  // Check for day ranges: e.g. "sun-thu", "sun–thu", "sun to thu", "mo-fr"
  const rangeMatch = lower.match(
    /\b(sun(?:day)?|mon(?:day)?|tue(?:sday)?|wed(?:nesday)?|thu(?:rsday)?|fri(?:day)?|sat(?:urday)?|su|mo|tu|we|th|fr|sa)\s*(?:[-–—~]|to)\s*(sun(?:day)?|mon(?:day)?|tue(?:sday)?|wed(?:nesday)?|thu(?:rsday)?|fri(?:day)?|sat(?:urday)?|su|mo|tu|we|th|fr|sa)\b/,
  );

  if (rangeMatch) {
    const startDay = DAY_MAP[rangeMatch[1]];
    const endDay = DAY_MAP[rangeMatch[2]];

    if (startDay !== undefined && endDay !== undefined) {
      const days: number[] = [];
      let curr = startDay;
      while (true) {
        days.push(curr);
        if (curr === endDay) break;
        curr = (curr + 1) % 7;
      }
      return days;
    }
  }

  // Check for single days: e.g. "fri", "friday", "sat", "sunday"
  const singleMatches = lower.match(
    /\b(sun(?:day)?|mon(?:day)?|tue(?:sday)?|wed(?:nesday)?|thu(?:rsday)?|fri(?:day)?|sat(?:urday)?)\b/g,
  );

  if (singleMatches && singleMatches.length > 0) {
    const days: number[] = [];
    for (const match of singleMatches) {
      const d = DAY_MAP[match];
      if (d !== undefined && !days.includes(d)) {
        days.push(d);
      }
    }
    return days;
  }

  return null;
}

/**
 * Determines whether a place's working hours are currently active (not over).
 */
export function isWithinWorkingHours(
  openingHours?: string,
  referenceDate = new Date(),
  timeZone = 'Asia/Jerusalem',
): boolean {
  if (!openingHours || !openingHours.trim()) {
    return false;
  }

  const text = openingHours.trim();
  const lower = text.toLowerCase();

  // 1. Check for 24/7 or continuous emergency services
  if (
    lower.includes('24/7') ||
    lower.includes('24 hours') ||
    lower.includes('24h') ||
    lower.includes('open 24') ||
    lower.includes('critical care') ||
    lower.includes('live on-duty ambulatory') ||
    lower.includes('rapid ambulatory house calls') ||
    lower.includes('emergency surgery') ||
    lower.includes('emergency triage')
  ) {
    return true;
  }

  const { day: currentDay, minutes: currentMinutes } = getLocalTimeDetails(
    referenceDate,
    timeZone,
  );

  // 2. Split into segments: e.g. "Sun–Thu 09:00–18:00 · Fri 09:00–13:00"
  const segments = text.split(/[·;|\n]+/).map((s) => s.trim()).filter(Boolean);

  let parsedAnyDay = false;

  for (const segment of segments) {
    const targetDays = parseDaysFromSegment(segment);

    // If segment defines specific days, only proceed if today is in those days
    if (targetDays && targetDays.length > 0) {
      parsedAnyDay = true;
      if (!targetDays.includes(currentDay)) {
        continue;
      }
    }

    // Look for all time ranges in this segment: e.g. "09:00–18:00", "08:00–11:00, 17:00–19:30"
    const timeRegex = /(\d{1,2}):(\d{2})\s*(?:[-–—~]|to)\s*(\d{1,2}):(\d{2})/g;
    let match: RegExpExecArray | null;

    while ((match = timeRegex.exec(segment)) !== null) {
      const startH = parseInt(match[1], 10);
      const startM = parseInt(match[2], 10);
      const endH = parseInt(match[3], 10);
      const endM = parseInt(match[4], 10);

      const startTotal = startH * 60 + startM;
      const endTotal = endH * 60 + endM;

      if (endTotal > startTotal) {
        // Normal daytime hours
        if (currentMinutes >= startTotal && currentMinutes < endTotal) {
          return true;
        }
      } else {
        // Overnight hours (e.g. 20:00 to 02:00)
        if (currentMinutes >= startTotal || currentMinutes < endTotal) {
          return true;
        }
      }
    }
  }

  // If we parsed day schedules and none matched the current time, the place's working hours are over / not open now
  if (parsedAnyDay) {
    return false;
  }

  // If no day/time pattern could be extracted, return false if hours unavailable
  if (lower.includes('unavailable') || lower.includes('check open hours')) {
    return false;
  }

  return true;
}

/**
 * Calculates whether a place is considered 'Open Now':
 * 1. Declared open through the portal (overrides standard schedule)
 * 2. Or the place's working hours are not over
 */
export function isPlaceOpenNow(
  place: PlaceOpenStatusInput,
  referenceDate = new Date(),
  timeZone = 'Asia/Jerusalem',
): boolean {
  // 1. Explicit portal declaration has highest priority
  if (
    place.portalStatusOverride === 'open' ||
    place.isDeclaredOpen === true
  ) {
    return true;
  }

  if (
    place.portalStatusOverride === 'closed' ||
    place.isDeclaredOpen === false ||
    place.capacityStatus === 'at_capacity'
  ) {
    return false;
  }

  // 2. Working hours schedule
  if (place.openingHours) {
    return isWithinWorkingHours(place.openingHours, referenceDate, timeZone);
  }

  // 3. Fallback to existing flag
  return place.isOpenNow ?? true;
}
